...
def comparison_plot(strings: dict,
    ...
    plt.hlines(mean_par+std_par, series_par.index.values[0], series_par.index.values[-1], color=strings["color_par"], alpha=0.4)
    plt.hlines(mean_seq+std_seq, series_seq.index.values[0], series_seq.index.values[-1], color=strings["color_seq"], alpha=0.4)
    plt.hlines(mean_par-std_par, series_par.index.values[0], series_par.index.values[-1], color=strings["color_par"], alpha=0.4)
    plt.hlines(mean_seq-std_seq, series_seq.index.values[0], series_seq.index.values[-1], color=strings["color_seq"], alpha=0.4)
    # box between
    plt.fill_between([0, series_par.index.values[-1]],
                     [mean_par+std_par, mean_par+std_par],
                     [mean_par-std_par, mean_par-std_par],
                     color=strings["color_par"], alpha=0.3)
    plt.fill_between([0, series_seq.index.values[-1]],
                     [mean_seq+std_seq, mean_seq+std_seq],
                     [mean_seq-std_seq, mean_seq-std_seq],
                     color=strings["color_seq"], alpha=0.3)
    # values
...
def complex_plot(strings: dict,
    ...
    ax.set_xscale("log")
    ax.set_yscale("log")
    plt.errorbar(iter_list, means_par, yerr=std_par, color=strings["color_par"],fmt=strings["marker_par"], label="mean parallel")
    plt.errorbar(iter_list, means_seq, yerr=std_seq, color=strings["color_seq"], fmt=strings["marker_seq"], label="mean sequential")
    plt.legend()
...
def complex_plots(df_10k: pd.DataFrame,
    ...
    for df in df_list:
        ...
        std_par.append(np.std(df["count_par"]))
        std_seq.append(np.std(df["count_seq"]))
    strings = get_strings("complex_count")
    complex_plot(strings, means_par, means_seq, std_par, std_seq, iter_list)
